                            ...
                            <div class="content rounded-md bg-opacity-50 mt-2">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="font-bold">{{
                                                `Tratamiento: ` }}<span class="font-normal">{{
                                                    capitalizeFirstLetter(card.treatment
                                                        === "" ?
                                                        "normal"
                                                        :
                                            ...
                                            <div class="flex flex-col gap-1">
                                                ...
                                                <span class=" flex flex-row gap-2 font-bold text-lg">{{ `Estado:
                                                    `
                                                            ...
                                                            :src="`/images/${parseInt(card.sold) === parseInt(card?.quantity || card?.qty) ? 'check-mark' : 'letter-x'}.png`"
                                ...
                                <div class="flex flex-row justify-end p-4 gap-2 col-span-full w-full">
                                    ...
                                    <Button v-if="tabs[activeTab].value !== 'complete'" size="small"
                                        @click="warningOnClose" :loading="loading" color="#E7000B">Cerrar
                                        orden</Button>
                                    <Button v-if="tabs[activeTab].value !== 'complete'" size="small"
                                        @click="markAllAsSold(sale)" :loading="loading"
                                        :disabled="sale.cart.some(x => x.sold)">Agregar todo</Button>
                                    <Button v-if="tabs[activeTab].value !== 'complete'" size="small"
                                        @click="markAllAsSold(sale, true)" :loading="loading"
                                        :disabled="sale.cart.some(x => !x.sold)">Quitar todo</Button>
...
function goWpp(phone) {
    openWhatsApp(`Hola, te hablo por tu pedido de ${capitalizeFirstLetter(props.game)}`, " ", phone)
}
async function init() {
    orderResumen.value = await fetchSalesResumen(props.game);
    await loadSales();
}
async function loadSales() {
    fetching.value = true;
    await fetchSales({ status: tabs.value[activeTab.value]?.value, search: searchOrder.value, game: props.game });
    fetching.value = false;
}
function changeSoldQty(card, qty) {
    if (card.sold === 0 && qty === -1) return;
    if (card.sold === (card.quantity || card.qty) && qty === 1) return;
    card.sold += qty;
}
function markAllAsSold(sale, remove = false) {
    if (remove) {
        sale.cart.forEach(x => x.sold = 0);
        return;
    }
    sale.cart.forEach(x => x.sold = (x.quantity || x.qty));
}

async function warningOnClose(sale) {
    if (confirm("⚠ ⚠ ¿Estás seguro que deseas cerrar esta orden? ⚠ ⚠\n\n\nLa orden se marcará como completada y no podrás modificarla nuevamente.")) {
        await confirmLocalOrder(sale, true)
    }
}
